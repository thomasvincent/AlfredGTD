...
# Print test header
echo -e "${YELLOW}Running tests for add_task.scpt...${NC}"
...
if [ -f "$SCRIPT_DIR/add_task.scpt" ]; then
...
if grep -q "tell application \"Reminders\"" "$SCRIPT_DIR/add_task.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
if grep -q "display notification" "$SCRIPT_DIR/add_task.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
if grep -q "addTaskToThings" "$SCRIPT_DIR/add_task.scpt" && \
   grep -q "addTaskToOmniFocus" "$SCRIPT_DIR/add_task.scpt" && \
   grep -q "addTaskToTodoist" "$SCRIPT_DIR/add_task.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
if grep -q "encodeText" "$SCRIPT_DIR/add_task.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
