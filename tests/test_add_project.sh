#!/bin/bash
# Test script for add_project.scpt
...
# Print test header
echo -e "${YELLOW}Running tests for add_project.scpt...${NC}"
...
if [ -f "$SCRIPT_DIR/add_project.scpt" ]; then
...
if grep -q "tell application \"Reminders\"" "$SCRIPT_DIR/add_project.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
if grep -q "display notification" "$SCRIPT_DIR/add_project.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
if grep -q "addProjectToThings" "$SCRIPT_DIR/add_project.scpt" && \
   grep -q "addProjectToOmniFocus" "$SCRIPT_DIR/add_project.scpt" && \
   grep -q "addProjectToTodoist" "$SCRIPT_DIR/add_project.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
...
# Test 5: Check for URL encoding function
echo -n "Test 5: Check for URL encoding function... "
if grep -q "encodeText" "$SCRIPT_DIR/add_project.scpt"; then
    echo -e "${GREEN}PASSED${NC}"
    PASSED=$((PASSED+1))
else
    echo -e "${RED}FAILED${NC} - Missing URL encoding function"
    FAILED=$((FAILED+1))
fi
